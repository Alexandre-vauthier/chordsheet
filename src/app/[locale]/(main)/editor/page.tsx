...
const PATH = '/editor';
const NS = 'Editorial.editorGuide';

/** Sections dans l'ordre de lecture. Chaque entrée attend une clé `h2` et une clé `body`. */
const SECTIONS = ['sections', 'measures', 'durations', 'keyboard', 'notation', 'mic', 'finder', 'custom', 'metadata'] as const;

export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'Seo.pages.editor' });
  const title = t('title');
    ...
    <Editorial bordered={false}>
      ...
      <EditorialLinks
        ...
        links={[
          ...
          { href: '/transpose', label: t('links.transpose') },
